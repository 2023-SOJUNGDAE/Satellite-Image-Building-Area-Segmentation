...
train_dataloader = DataLoader(train_dataset, batch_size=32, shuffle=True, num_workers=24)
valid_dataloader = DataLoader(valid_dataset, batch_size=32, shuffle=False, num_workers=16)

backbone = "upernet-convnext-large"
model = UperNetForSemanticSegmentation.from_pretrained(f"openmmlab/{backbone}", num_labels=1, ignore_mismatched_sizes=True).to(device)
    ...
    with tqdm(
        ...
        for images, masks in iterator:
            ...
            for i in range(num_patches):
                ...
                iou_score = iou_score_calculator(output.logits, mask.unsqueeze(1)).cpu().detach().numpy()
        ...
        with tqdm(
            ...
            for val_images, val_masks in iterator:
                ...
                for i in range(val_num_patches):
                    ...
                    val_loss = criterion(val_output.logits, val_mask.unsqueeze(1))
                    ...
                    iou_score = iou_score_calculator(val_output.logits, val_mask.unsqueeze(1)).cpu().detach().numpy()
                    iou_meter.add(iou_score)
                    iou_logs = {'iou_score': iou_meter.mean}
