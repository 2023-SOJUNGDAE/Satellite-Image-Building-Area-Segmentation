...
train_transform = A.Compose(
    [
        ...
        A.RandomBrightnessContrast(p=0.4), # 0.3 init
        A.RandomShadow(shadow_roi=(0, 0, 1, 1), p=0.3), # 0.3 init

        A.GridDistortion(p=0.3), # 0.2 init
        A.OneOf([
...
model_name = "/home/ubin108/data/unet_convNext_base_final.pth"
...
scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
    optimizer, T_0=1, T_mult=2, eta_min=5e-7,
)
...
for epoch in range(max_epochs):  # epoch
    ...
    torch.save(model.state_dict(), f'./convNext/convNext_base_epoch_{epoch+1}.pth')
