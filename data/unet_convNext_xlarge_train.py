...
train_dataloader = DataLoader(train_dataset, batch_size=16, shuffle=True, num_workers=32)
valid_dataloader = DataLoader(valid_dataset, batch_size=16, shuffle=False, num_workers=32)
...
model_name = "/home/ubin108/data/unet_convNext_xlarge_final.pth"
weights = torch.load(f'{model_name}', map_location=device)
new_state_dict = {k.replace('module.', ''): v for k, v in weights.items()}
model.load_state_dict(new_state_dict)

criterion = dice_loss()
init_lr = 1e-5  # 초기 학습률 설정
optimizer = torch.optim.AdamW(model.parameters(), lr=init_lr, weight_decay=1e-9)
max_epochs = 10
scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
    optimizer, T_0=10, T_mult=2, eta_min=2e-6,
)
...
for epoch in range(max_epochs):  # epoch
    ...
    torch.save(model.state_dict(), f'./convNext/convNext_xlarge_all_epoch_{epoch+1}.pth')
